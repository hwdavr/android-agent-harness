...
*   **Action**:
    1. **INVOKE** the `android-testing` skill via the Skill tool (name: `android-testing`). Reading the SKILL.md manually is not a substitute — the Skill tool is the required mechanism. Implement every selected `Acceptance Test Cases` row, including its named method and shared JSON scenario. If the selected feature's `production_journey.required` is `true`, implement its named journey acceptance-test owner through the declared production entry point, real UI actions, return boundary, and visible post-return assertion; do not replace it with a ViewModel or `*Content` test.
...
*   **Action**:
    1. **INVOKE** the `android-testing` skill via the Skill tool (name: `android-testing`). Reading the SKILL.md manually is not a substitute — the Skill tool is the required mechanism. Execute and verify every `Acceptance Test Cases` row in the selected user story. When `production_journey.required` is `true`, run the named journey owner through the declared production entry point and verify the declared real actions, return boundary, and visible post-return assertion with `bash harness/scripts/check-journey-test-contract.sh`; a direct ViewModel or `*Content` test is supplemental only. The primary acceptance test must exercise the production entry point; an isolated helper or use-case test cannot substitute for user-visible or cross-layer behavior. Verify through the actual UI/API and meet code coverage targets (overall project **≥ 80%**, ViewModel & Use Case **≥ 90%**). Read the approved Rule Applicability decisions and preserve evidence for every `Required` row.
