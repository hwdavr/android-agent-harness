...
- **Reference resolution is deterministic and explicit** in batch mode (`--feature`): an explicit `visual_evidence/reference-map.json` entry takes precedence, then an exact-name golden baseline, then the most specific `design/mockup_*.png` token match (highest token overlap, then fewest unmatched reference tokens), then the anchor report's declared reference. Pairings are never chosen by filesystem iteration order, and a capture with no resolvable reference fails the gate as `NO_REFERENCE` (exit 2) instead of being silently skipped.
