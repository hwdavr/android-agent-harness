# Requirement Summary — <Feature Name>

## Requirement Summary

<One paragraph describing the requested end-to-end outcome.>

## User Goal

As a <user type>, I want to <action> so that <outcome>.

## Expected Behavior

1. <observable and independently testable behavior>

## Business Rules

- <constraint or “None identified.”>

## Known Constraints

- <technical or platform constraint, such as minSdk 24>

## Non-Goals

- <explicitly out-of-scope behavior>

## Rule Applicability

Copy the complete matrix from
[`rule-applicability-template.md`](rule-applicability-template.md). Keep every row and
replace each `<decision>` with `Required`, `Not applicable — <feature-specific reason>`,
or `Exception — approved by <user/date>`.

| Rule ID | Rule document | Default | Decision for this change | Trigger / rationale | Planned evidence |
|---|---|---|---|---|---|
| ARCH | `android-architecture.md` | Always | <decision> | <trigger/rationale> | <evidence> |
| IMPL | `implementation-rules.md` | Always | <decision> | <trigger/rationale> | <evidence> |
| TEST | `testing-strategy.md` | Always | <decision> | <trigger/rationale> | <evidence> |
| SUI | `compose-rules.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |
| L10N | `localization-rules.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |
| NAV | `navigation-rules.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |
| API | `api-contract-rules.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |
| OBS | `observability.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |
| ANL | `analytics-rules.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |
| SEC | `android-security.md` | Conditional | <decision> | <trigger/rationale> | <evidence> |

## Explicit Assumptions

1. <assumption and risk>

## Open Questions

| # | Question | Status | Answer |
|---|---|---|---|
| Q1 | <question> | ⚠️ Unanswered / ✅ Answered | <answer> |
